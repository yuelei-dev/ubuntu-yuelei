...
class ScriptActionsUiTests(unittest.TestCase):
    ...
    def test_copy_breakdown_image_and_video_submissions_are_idempotent(self):
        ...
        self.assertIn("'Idempotency-Key':localPending.key", self.html)
        self.assertIn("'script-breakdown-local-'+mediaType", self.html)
        self.assertIn("file_size:Number(file.size||0)", self.html)
        self.assertIn("last_modified:Number(file.lastModified||0)", self.html)
    ...
    def test_lost_submission_response_reuses_pending_key(self):
        ...
        self.assertIn("saved&&saved.body===body&&saved.key", self.html)
        self.assertIn("return {storageKey:storageKey,body:body,key:saved.key,reused:true}", self.html)
        self.assertIn("key:pending.key,reused:false", self.html)
        self.assertIn("fetch('/api/gen/breakdown'", self.html)
