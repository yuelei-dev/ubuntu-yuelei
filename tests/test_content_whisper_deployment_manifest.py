...
DEPLOYMENT_TARGET_COMMIT = "69ffd43e07a28ef8827d04cd4663028ad71874c9"
LOCKED_PREIMAGES = {
    "server/content_domains/video.py": (
        "file",
...
class ContentWhisperDeploymentManifestTests(unittest.TestCase):
    ...
    def test_locked_preimages_match_current_mixed_test_runtime(self):
        actual = {
        ...
        self.assertTrue(all(state == "file" for state, _, _ in actual.values()))
        self.assertEqual(
            {
                entry["repository_path"]
                for entry in self.manifest["files"]
                if entry["target_preimage_sha256"] == entry["expected_postimage_sha256"]
            },
            {
                "server/content_domains/script_to_video.py",
                "deploy/systemd/huangque-content.service.d/whisper.conf",
        ...
        self.assertEqual(
            self.manifest["source"]["deployment_target_commit"],
            DEPLOYMENT_TARGET_COMMIT,
        )
    ...
    def test_verifier_rejects_unexpected_file_for_absent_preimage(self):
        ...
        absent_entry["target_preimage_sha256"] = synthetic[
