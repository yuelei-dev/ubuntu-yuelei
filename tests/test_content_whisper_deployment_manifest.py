...
class ContentWhisperDeploymentManifestTests(unittest.TestCase):
    ...
    def test_manifest_reconciles_observed_mixed_state_to_pr253_merge(self):
        self.assertEqual(
            "69ffd43e07a28ef8827d04cd4663028ad71874c9",
            self.manifest["source"]["runtime_target_commit"],
        )
        evidence = self.manifest["known_test_runtime_evidence"]
        self.assertTrue(evidence["reconciliation_preimages_captured_read_only"])
        mixed = evidence["reconciliation_mixed_state"]
        self.assertEqual(2, mixed["intermediate_targets"])
        self.assertEqual(5, mixed["already_final_targets"])
        self.assertTrue(mixed["service_active"])
        self.assertEqual(200, mixed["health_status"])
        self.assertEqual(401, mixed["unauthenticated_history_status"])
        self.assertFalse(mixed["writes_or_restarts_during_capture"])
        self.assertEqual(
            2,
            sum(
                entry["target_preimage_sha256"]
