...
- 支持 OpenRouter 文生视频、图生视频、任务轮询和鉴权下载。
...
采用主站 #721 与 #722 的备用接口部分。相比全量切换 OpenRouter，它保留 xAI 主通道；相比只捕获 HTTP 403 的临时处理，它同时覆盖下载鉴权和非幂等安全边界。克隆仓库尚未拆分主站的启动恢复模块，本次不跨组移植重启恢复。
...
4. OpenRouter 创建成功后保存任务 ID 和 `openrouter_*` 阶段，再进入轮询。
5. OpenRouter 完成后返回受保护的结果地址，下载时仅给原始 OpenRouter 地址附加 Bearer 鉴权；代理或其他候选下载地址不得携带该密钥。
...
- 修改 `server/content_domains/video_xai.py`、`server/content_domains/video.py`。
...
5. OpenRouter 结果下载只对原始地址附加鉴权，密钥不进入代理候选请求。
6. 现有果肉视频、计费、退点和模块门禁测试全部通过。
...
部署前备份三个后端文件和环境文件。出现回归时恢复文件与环境备份、重启 `huangque-content.service`，即可回到 xAI 单通道行为。

## 后续事项

克隆仓库当前在服务重启后由 `core.py` 将遗留的 running 任务判失败退点。主站的按提供商恢复机制涉及不同组文件，若需要同步，应另开独立规格与 PR；本次不修改该行为。
