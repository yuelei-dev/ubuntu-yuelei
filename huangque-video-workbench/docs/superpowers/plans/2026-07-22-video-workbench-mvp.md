...
it('uses avatar for intro/transition/CTA and visual media for product demonstration', () => {
  const board = buildStoryboard({title: '产品介绍', script: '大家好，我是子辰。这个产品有三个特点。先看一下产品实拍。第一个特点是使用简单。点击关注，下期见。'});
...
export const classifySegment = (text: string, index: number, total: number): Scene['type'] => {
  ...
  if (/实拍|外观|展示|画面|操作过程/u.test(text)) return 'image_video';
  if (/数据|增长|下降|比例|对比/u.test(text)) return 'chart';
  return 'avatar';
};
```
...
it('creates a project and returns without waiting for generation', async () => {
  const response = await app.inject({method: 'POST', url: '/api/projects', payload: {input: {type: 'script', content: '大家好。展示一下产品。点击关注。'}, avatar: {avatarId: 'mock', voiceId: 'mock'}, output: {templateId: 'vertical_knowledge_v1'}}});
...
it('regenerates one scene without submitting the full project', async () => {
  ...
  await user.click(screen.getByRole('button', {name: '重新生成 scene_002'}));
...
test('script becomes a completed mixed-scene preview', async ({page}) => {
  ...
  await page.getByLabel('口播文案').fill('大家好，我是子辰。下面展示产品实拍。第一个特点是使用简单。点击关注，下期见。');
