...
export const probeMedia = async (path: string): Promise<MediaProbe> => {
  try {
    ...
    const {stdout} = await runMediaCommand('ffprobe', ['-v', 'error', '-show_entries', 'format=duration:stream=codec_type,codec_name,width,height,avg_frame_rate,r_frame_rate', '-of', 'json', input]);
