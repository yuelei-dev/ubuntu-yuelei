...
const inspectBlackFrames = async (path: string, durationMs: number): Promise<number> => {
  const {stderr} = await runMediaCommand('ffmpeg', ['-nostdin', '-v', 'info', '-i', path, '-an', '-vf', 'blackdetect=d=0.1:pic_th=0.98:pix_th=0.10', '-f', 'null', '-']);
  return ratioFromLog(stderr, 'black_duration', durationMs);
};

const inspectSilence = async (path: string, durationMs: number): Promise<number> => {
  const {stderr} = await runMediaCommand('ffmpeg', ['-nostdin', '-v', 'info', '-i', path, '-vn', '-af', 'silencedetect=noise=-50dB:d=0.5', '-f', 'null', '-']);
...
export const inspectOutput = async (
  ...
  if (probe) {
    ...
    try {
      metrics.blackFrameRatio = await inspectBlackFrames(validateMediaPath(path), probe.durationMs);
      if (metrics.blackFrameRatio > (expected.maxBlackFrameRatio ?? 0.5)) errors.push('black-frame ratio exceeds allowed threshold');
      if (probe.audio) {
        metrics.silenceRatio = await inspectSilence(validateMediaPath(path), probe.durationMs);
