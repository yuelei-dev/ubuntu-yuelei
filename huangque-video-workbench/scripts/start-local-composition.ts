...
export const fixtureAttemptDirectory = (runDirectory: string, projectId: string, attemptId: string): string =>
  resolve(runDirectory, projectId, attemptId);
...
const renderFixtureVideoWithinDeadline = async ({
  ...
  const projectDirectory = fixtureAttemptDirectory(runDirectory, projectId, attemptId);
...
export const copyOutputWithinDeadline = async (
  source: string, destination: string, signal: AbortSignal, timeoutMs: number,
  copy: (from: string, to: string) => Promise<void> = copyFile
): Promise<void> => {
  await withDeadline('fixture final output copy', timeoutMs, async (deadlineSignal) => {
...
export const startLocalComposition = async ({port = 4173}: {port?: number} = {}): Promise<LocalComposition> => {
  ...
  const processor = createBullMqPipelineProcessor({
    ...
    qualityInspector: {
      async inspect(project, signal, attempt) {
        ...
        const renderSignal = signal ? AbortSignal.any([queue.signal, signal]) : queue.signal;
        const attemptId = attempt?.owner ?? 'default';
        const rendered = await renderFixtureVideo({projectId: project.id, scenes, runDirectory, chromeExecutable, serveUrl, signal: renderSignal, attemptId});
        ...
        await assertFixtureVideoVisuals({videoPath: rendered.outputPath, ...sampleTimes, signal: renderSignal});
        renderSignal.throwIfAborted();
        if (report.passed) {
          const finalAttemptPath = resolve(dirname(finalOutputPath), project.id, attemptId, basename(finalOutputPath));
          await mkdir(dirname(finalAttemptPath), {recursive: true});
          await copyOutputWithinDeadline(rendered.outputPath, finalAttemptPath, renderSignal, fixtureDeadlines.muxMs);
          return {report, reportPath, previewUrl: `/api/projects/${project.id}/preview.mp4`, downloadUrl: `/api/projects/${project.id}/preview.mp4`, publish: () => { outputs.set(project.id, finalAttemptPath); }};
        }
  ...
  queue.setProcessor(async (delivery) => {
    await processor({...delivery, signal: queue.signal});
  });

  const app = createApp({
    repository: durableRepository,
    queue,
    // The self-contained fixture has no Huangque auth service. Keep the
    // bypass explicit and local to this deterministic test composition.
    authenticate: async () => ({username: 'fixture-user', role: 'tester'}),
    objectReader: {
      open: async (objectKey) => {
        const projectId = /^\/api\/projects\/([^/]+)\/preview\.mp4$/.exec(objectKey)?.[1];
        const outputPath = projectId ? outputs.get(projectId) : undefined;
        if (!outputPath) throw new Error('fixture output not found');
  ...
  const html = '<!doctype html><html lang="en"><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width,initial-scale=1"><meta name="workbench-ui-base" content=""><meta name="workbench-api-base" content="/api"><title>Huangque Video Workbench</title></head><body><div id="root"></div><script type="module" src="/fixture-client.js"></script></body></html>';
