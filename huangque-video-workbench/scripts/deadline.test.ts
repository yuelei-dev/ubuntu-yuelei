...
import {copyOutputWithinDeadline} from './start-local-composition.js';
...
describe('fixture deadlines', () => {
  ...
  it('bounds a never-settling local output copy', async () => {
    await expect(copyOutputWithinDeadline('source.mp4', 'output.mp4', new AbortController().signal, 10,
      async () => await new Promise(() => undefined))).rejects.toBeInstanceOf(FixtureTimeoutError);
  });
});
