...
export const FIXTURE_SCRIPT = '大家好，我是子辰。下面展示产品实拍。第一个特点是使用简单。点击关注，下期再见。';
  ...
  return withDeadline(`project ${projectId} HTTP polling`, timeoutMs, async (signal) => {
    for (;;) {
      const project = await readProject(await fetcher(
        `${baseUrl}${apiBasePath}/projects/${encodeURIComponent(projectId)}`,
        {method: 'GET', signal}
      ...
      if (terminalStatuses.has(project.status)) {
        ...
        const authenticatedOutputUrl = `${apiBasePath}/projects/${encodeURIComponent(projectId)}/output`;
        if (project.previewUrl !== authenticatedOutputUrl && !project.previewUrl?.endsWith('/preview.mp4')) {
          throw new Error(`project ${projectId} completed without an authenticated preview MP4 URL`);
        }
        return project;
      }
...
export const runFixtureProject = async ({
  ...
  timeoutMs = 120_000,
  fetcher = fetch,
  generation = {avatarId: 'mock', voiceId: 'mock', templateId: 'vertical_knowledge_v1'}
}: {
  baseUrl: string;
  apiBasePath?: string;
  timeoutMs?: number;
  fetcher?: Fetcher;
  generation?: {avatarId: string; voiceId: string; templateId: string};
}): Promise<FixtureProject> => {
  return withDeadline('fixture project creation and completion', timeoutMs, async (signal) => {
    const startedAt = Date.now();
    const response = await fetcher(`${baseUrl}${apiBasePath}/projects`, {
      method: 'POST',
      headers: {'content-type': 'application/json'},
      body: JSON.stringify({
        input: {type: 'script', content: FIXTURE_SCRIPT},
        avatar: {avatarId: generation.avatarId, voiceId: generation.voiceId},
        output: {templateId: generation.templateId}
      }),
      signal
    });
    const created = await readProject(response, 'project creation');
    const remainingMs = Math.max(1, timeoutMs - (Date.now() - startedAt));
    return waitForTerminalProject({baseUrl, apiBasePath, projectId: created.id, timeoutMs: remainingMs, fetcher});
  });
};
