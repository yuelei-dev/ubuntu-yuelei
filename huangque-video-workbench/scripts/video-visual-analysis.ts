...
const visualAnalysisDeadlineMs = 15_000;
...
const extractFrame = (videoPath: string, timeMs: number, signal?: AbortSignal): Promise<Buffer> => new Promise((resolve, reject) => {
  execFile('ffmpeg', [
    '-v', 'error', '-ss', (timeMs / 1000).toFixed(3), '-i', videoPath,
    '-frames:v', '1', '-vf', `scale=${frameWidth}:${frameHeight}`,
    '-pix_fmt', 'rgb24', '-f', 'rawvideo', 'pipe:1'
  ], {encoding: 'buffer', maxBuffer: frameWidth * frameHeight * 4, windowsHide: true, signal}, (error, stdout, stderr) => {
    if (error) {
...
export const analyzeVideoFrame = async (videoPath: string, timeMs: number, signal?: AbortSignal): Promise<VideoFrameAnalysis> => {
  signal?.throwIfAborted();
  const frame = await extractFrame(videoPath, timeMs, signal);
  return {
...
export const analyzeVideoFrameWithinDeadline = async (
  videoPath: string,
  timeMs: number,
  signal: AbortSignal,
  timeoutMs = visualAnalysisDeadlineMs,
  analyzeFrame: (path: string, sampleTimeMs: number, analysisSignal: AbortSignal) => Promise<VideoFrameAnalysis> = analyzeVideoFrame
): Promise<VideoFrameAnalysis> => await withDeadline('fixture visual frame analysis', timeoutMs, async (deadlineSignal) => {
  const combined = AbortSignal.any([signal, deadlineSignal]);
  combined.throwIfAborted();
  const frame = await analyzeFrame(videoPath, timeMs, combined);
  combined.throwIfAborted();
  return frame;
});
...
export const assertFixtureVideoVisuals = async ({
  ...
  const analysisSignal = signal ?? new AbortController().signal;
  const avatar = await analyzeVideoFrameWithinDeadline(videoPath, avatarTimeMs, analysisSignal);
  const broll = await analyzeVideoFrameWithinDeadline(videoPath, brollTimeMs, analysisSignal);
