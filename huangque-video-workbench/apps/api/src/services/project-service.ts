...
export interface ProjectRepository {
  ...
  findProject(id: string): Promise<ProjectDetail | undefined>;
  ...
  activateSceneRegeneration(projectId: string, sceneId: string, expectedGenerationJobId: string | undefined, nextGenerationJobId: string, nextProjectStatus: RegenerationProjectStatus): Promise<SceneRegenerationActivationResult | undefined>;
  claimSceneActiveGenerationJob(projectId: string, sceneId: string, expectedGenerationJobId: string | undefined, nextGenerationJobId: string): Promise<SceneRecord | undefined>;
  beginSceneGeneration(projectId: string, sceneId: string, generationJobId: string): Promise<SceneRecord | undefined>;
  completeSceneGeneration(projectId: string, sceneId: string, generationJobId: string, asset: AssetVersionWrite): Promise<AssetVersionRecord | undefined>;
  updateScene(projectId: string, sceneId: string, patch: ScenePatch): Promise<SceneRecord | undefined>;
...
export class InMemoryProjectRepository implements ProjectRepository {
  ...
  async findProject(id: string): Promise<ProjectDetail | undefined> {
    ...
    return {
      ...clone(project),
      scenes: [...this.sceneRecords.values()].filter((scene) => scene.projectId === id).sort((a, b) => a.order - b.order).map(clone),
      jobs: [...this.jobRecords.values()].filter((job) => job.projectId === id).map(clone),
      assetVersions: [...this.assetVersionRecords.values()].filter((asset) => asset.projectId === id).map(clone)
    };
  }
  ...
  async activateSceneRegeneration(
    ...
    if (!project || !scene ||
        ...
        scene.visual.activeGenerationJobId !== expectedGenerationJobId) return undefined;
    const now = new Date().toISOString();
    ...
    const activatedScene = {
      ...sceneWithoutFailure,
      status: 'PENDING',
      visual: {...scene.visual, activeGenerationJobId: nextGenerationJobId},
      updatedAt: now
    };
  ...
  async claimSceneActiveGenerationJob(
    ...
    if (!scene || scene.visual.activeGenerationJobId !== expectedGenerationJobId) return undefined;
    const updated = {
      ...scene,
      visual: {...scene.visual, activeGenerationJobId: nextGenerationJobId},
  ...
  async completeSceneGeneration(
    ...
    const key = `${projectId}:${sceneId}`;
    const scene = this.sceneRecords.get(key);
    if (!scene || scene.visual.activeGenerationJobId !== generationJobId || scene.status !== 'GENERATING') return undefined;
  ...
  async markJobQueued(id: string): Promise<void> {
    ...
    this.jobRecords.set(id, {...job, status: 'QUEUED'});
  }
...
export class ProjectService {
  ...
  async patchScene(projectId: string, sceneId: string, patch: ScenePatch): Promise<SceneRecord | undefined> {
    return this.repository.updateScene(projectId, sceneId, patch);
  }
  ...
  private jobFor(project: ProjectRecord, sceneId: string, taskType: string, input: unknown): JobRecord {
    ...
    return {id: jobKey(project.id, sceneId, taskType, inputHash), projectId: project.id, sceneId, taskType, inputHash, status: 'PENDING', createdAt: this.now().toISOString()};
  }
  ...
  private async enqueue(job: JobRecord, input: unknown): Promise<QueueSubmission> {
    if (job.status === 'QUEUED') return {id: job.id, existing: true};
    ...
    await this.repository.markJobQueued(job.id);
