...
export const createProjectApi = ({
  ...
  const normalizedApiBasePath = `/${apiBasePath.split('/').filter(Boolean).join('/')}`;
  const path = (suffix: string) => `${baseUrl}${normalizedApiBasePath}${suffix}`;
  ...
  return {
    async createProject(input) {
      const response = await request('/projects', 'POST', input);
