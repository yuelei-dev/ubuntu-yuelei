...
import {MockAvatarProvider, MockImageProvider, type AvatarProvider, type ImageProvider} from '@huangque/providers';
...
describe('runProjectPipeline', () => {
  ...
  it('publishes successful generation and asset versions but not a failed generation claim', async () => {
    ...
    const repository = new Task4PipelineRepository(durable, () => new Date(), broker);
    expect(await repository.beginSceneGeneration('project_mutations', 'scene_001', 'job_1')).toBe(true);
    expect(await repository.beginSceneGeneration('project_mutations', 'scene_001', 'job_wrong')).toBe(false);
