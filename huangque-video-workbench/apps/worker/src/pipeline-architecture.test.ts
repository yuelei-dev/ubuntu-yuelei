...
import {
  ...
  CanonicalSceneQuarantinedError,
...
describe('pipeline architecture', () => {
  it('builds the final-render scene view from canonical fields during the workerScene transition', async () => {
    const task4 = new InMemoryProjectRepository();
    await task4.createProject(projectRecord('COMPLETED'));
    const now = new Date(0).toISOString();
    await task4.createScene({
      id: 'scene_legacy', projectId: 'project_001', order: 1, status: 'READY', script: 'Edited narration',
      visual: {
        layout: 'visual_full', highlightWords: [], visualPrompt: 'Edited direction', contentRevision: 2,
        sceneType: 'image', purpose: 'visual', durationEstimate: 2,
        workerScene: {...imageScene('scene_legacy'), script: 'Stale narration'}
      },
      createdAt: now, updatedAt: now
    });

    const renderScenes = await new Task4PipelineRepository(task4, () => new Date(0)).listScenes('project_001');

    expect(renderScenes).toMatchObject([{script: 'Edited narration', visualPrompt: 'Edited direction', contentRevision: 2}]);
  });

  it('rejects an invalid transitional workerScene through the controlled quarantine error', async () => {
    const task4 = new InMemoryProjectRepository();
    await task4.createProject(projectRecord('COMPLETED'));
    const now = new Date(0).toISOString();
    await task4.createScene({
      id: 'scene_invalid', projectId: 'project_001', order: 1, status: 'READY', script: 'Narration',
      visual: {layout: 'visual_full', highlightWords: [], workerScene: {type: 'invalid', purpose: '', durationEstimate: -1}},
      createdAt: now, updatedAt: now
    });

    await expect(new Task4PipelineRepository(task4).listScenes('project_001')).rejects.toThrow(CanonicalSceneQuarantinedError);
  });
  ...
  it('emits persisted ALIGNING_TIMELINE only after every scene is READY or fallback-accepted', async () => {
    ...
    await task4.createScene({
      ...
      visual: {
        ...fallback.visual,
        sceneType: fallback.type,
        purpose: fallback.purpose,
        durationEstimate: fallback.durationEstimate,
        ...(fallback.visualPrompt === undefined ? {} : {visualPrompt: fallback.visualPrompt})
      },
      asset: null,
      createdAt: now,
      updatedAt: now
