...
_PRECISION_CONTRACT = "digital_human_precision_director_v5"
...
def _mapped_managed_directory_node(root, absolute_path):
    """Map an absent directory only through existing regular parents."""
    value = pathlib.PurePosixPath(str(absolute_path))
    if (not value.is_absolute() or ".." in value.parts
            or len(value.parts) < 2):
        raise ReleaseError("manifest runtime path must be absolute and normalized")
    root = pathlib.Path(root).resolve()
    parent = root
    for component in value.parts[1:-1]:
...
def _load_manifest(path):
    ...
    if executor.get("contract") != _PRECISION_CONTRACT:
        raise ReleaseError("Precision v5 executor rejects every other release contract")
    if tuple(manifest_path.parts[-3:]) != _PRECISION_MANIFEST_PARTS:
        raise ReleaseError("Precision v5 manifest must come from its locked source path")
...
def _validate_precision_contract(manifest):
    ...
    if not expected or actual != expected:
        raise ReleaseError("Precision v5 runtime inventory is incomplete")
    directories = manifest.get("directories")
    if not isinstance(directories, list) or len(directories) != 1:
        raise ReleaseError("Precision v5 managed directory contract is incomplete")
    directory = directories[0]
    preview_root = "/var/www/huangquechuanmei/assets/one-click/previews"
    ownership_source = "/var/www/huangquechuanmei/assets/one-click"
    preview_children = {
        item["runtime_path"] for item in manifest["files"]
        if item.get("repository_path", "").startswith(
            "site/assets/one-click/previews/"
        )
    }
    if (directory.get("runtime_path") != preview_root
            or directory.get("preimage_state") != "absent"
            or directory.get("postimage_state") != "directory"
            or directory.get("install_mode") != "0755"
            or directory.get("ownership_source_runtime_path") != ownership_source
            or set(directory.get("child_runtime_paths") or []) != preview_children
            or len(preview_children) != 3):
        raise ReleaseError("Precision v5 preview directory contract is invalid")
    for item in manifest["files"]:
        if item.get("runtime_path") in preview_children:
            if item.get("target_preimage_state") != "absent":
                raise ReleaseError("Precision v5 preview preimage must be absent")
    nginx = manifest.get("nginx_contract")
    if not isinstance(nginx, dict):
        raise ReleaseError("Precision v5 Yuelei Nginx contract is missing")
    if nginx.get("runtime_path") != "/etc/nginx/sites-available/yuelei-test.conf":
        raise ReleaseError("Precision v5 may only update the Yuelei test vhost")
    if nginx.get("enabled_runtime_path") != "/etc/nginx/sites-enabled/yuelei-test.conf":
        raise ReleaseError("Precision v5 Yuelei enabled-vhost path is invalid")
    if "huangquechuanmei" in str(nginx.get("runtime_path") or ""):
        raise ReleaseError("Precision v5 must not update the main-site vhost")
    if nginx.get("source_repository_path") != "deploy/nginx-huangquechuanmei.conf":
        raise ReleaseError("Precision v5 Nginx source path is invalid")
    if nginx.get("renderer_repository_path") != "deploy/render_yuelei_test_nginx.py":
        raise ReleaseError("Precision v5 Nginx renderer path is invalid")
    probe_paths = {
...
def _atomic_install(source, target, mode, replace, uid=None, gid=None):
    ...
    if not target.parent.is_dir() or target.parent.is_symlink():
        raise ReleaseError("atomic install parent is missing or unsafe")
...
def _verify_precision_checkout(source_root, manifest, reviewed_head, merged_main):
    ...
    allowed = {"deploy/test-runtime/digital-human-precision-director-v5-20260822.json"}
...
def _execute_precision_release(
    ...
    for item in manifest["directories"]:
        target = _mapped_managed_directory_node(
            target_root, item["runtime_path"],
        )
        if target.exists() or target.is_symlink():
            raise ReleaseError("managed directory preimage must be absent")
        ownership_source = _mapped_path(
            target_root, item["ownership_source_runtime_path"],
        )
        if (ownership_source != target.parent
                or not ownership_source.is_dir()
                or ownership_source.is_symlink()):
            raise ReleaseError("managed directory ownership source is unsafe")
        source_stat = ownership_source.stat()
        directories.append((
            item, target, int(item["install_mode"], 8),
            source_stat.st_uid, source_stat.st_gid,
        ))
    managed_parents = {
        target: (mode, uid, gid)
        for _, target, mode, uid, gid in directories
    }
    entries = []
    for item in manifest["files"]:
        source = (source_root / item["repository_path"]).resolve()
        ...
        if state == "absent":
            ...
            if target.parent in managed_parents:
                _, uid, gid = managed_parents[target.parent]
            else:
                if not target.parent.is_dir() or target.parent.is_symlink():
                    raise ReleaseError("new target parent is missing or unsafe")
                parent_stat = target.parent.stat()
    ...
    for item, _, mode, uid, gid in directories:
        audit["directories"].append({
            "runtime_path": item["runtime_path"],
            "preimage_state": item["preimage_state"],
            "postimage_state": item["postimage_state"],
            "mode": "%04o" % mode,
            "uid": uid,
            "gid": gid,
        })
    ...
    try:
        ...
        audit["directory_postimages"] = [
            {"runtime_path": item["runtime_path"], "state": "directory"}
            for item, _, _, _, _ in directories
        ]
        audit["final_files"] = [
        ...
        for item, target, mode, uid, gid in reversed(created_directories):
            try:
                ...
                target.rmdir()
                if target.exists() or target.is_symlink():
                    raise ReleaseError("managed directory preimage was not restored")
            except BaseException as error:
                rollback_errors.append("directory:" + type(error).__name__)
        try:
            hooks.validate_nginx()
            hooks.restart(service)
            hooks.reload_nginx()
