...
_PRECISION_CONTRACT = "digital_human_precision_director_v3"
...
def _load_manifest(path):
    ...
    if executor.get("contract") != _PRECISION_CONTRACT:
        raise ReleaseError("Precision v3 executor rejects every other release contract")
    if tuple(manifest_path.parts[-3:]) != _PRECISION_MANIFEST_PARTS:
        raise ReleaseError("Precision v3 manifest must come from its locked source path")
...
def _validate_precision_contract(manifest):
    ...
    if (not isinstance(timeout, (int, float)) or isinstance(timeout, bool)
        ...
        raise ReleaseError("Precision v3 rollback health timeout is invalid")
    if (not isinstance(interval, (int, float)) or isinstance(interval, bool)
            or interval <= 0 or interval > timeout):
        raise ReleaseError("Precision v3 rollback health interval is invalid")
