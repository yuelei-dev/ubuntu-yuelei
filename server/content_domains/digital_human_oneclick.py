...
def plan(script):
    ...
    gestures = (
        "人物保持与参考照片完全一致，竖屏半身口播照，右手自然抬起作开场讲解手势，左手放松，正视镜头",
        "人物保持与参考照片完全一致，竖屏半身口播照，双手在胸前自然展开作对比说明手势，正视镜头",
        "人物保持与参考照片完全一致，竖屏半身口播照，一手轻指前方作总结强调手势，另一手自然放松，正视镜头",
    )
    segments = []
    materials = []
...
def compose(payload, persist_state=None):
    ...
    for index, source in enumerate(videos):
        ...
        _run([
            ...
            "scale=1080:1920:force_original_aspect_ratio=increase,crop=1080:1920,setsar=1,fps=25",
    ...
    for index in range(MATERIAL_COUNT):
        ...
        filters.append(
            ...
            "d=1:s=1080x1920:fps=25,trim=duration=%.3f,setpts=PTS-STARTPTS[%s]" % (index, scene_duration, label)
        )
    filters.append("%sconcat=n=%d:v=1:a=0[outv]" % ("".join(labels), MATERIAL_COUNT))
    ...
    pip_filter = (
        "color=c=#B86B2B:s=456x456:r=25,format=rgba,"
    ...
    result = {
        ...
        "verification": {
            "resolution": "1080x1920", "subtitle": "whisper",
