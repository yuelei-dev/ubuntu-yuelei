...
def _set_terminal(job_id, status, result=None, error=None, from_states=("running",), cleanup_delay=0):
    claimed = jobs_store.set_terminal_with_video_outbox(
        jdb, job_id, status, result, error, from_states)
    if claimed:
        try:
            _domains()[2].cleanup_job_staged_seedance_references(
                job_id, delay_seconds=cleanup_delay)
        except Exception:
            pass
    return claimed
...
def _fail_job_and_schedule_refund(job_id, error, *, from_states=("running",),
    ...
    claimed = _set_terminal(
        job_id, "error", error=error, from_states=from_states,
...
def run_job(job_id):
    ...
    try:
        ...
        cleanup_delay = getattr(
            _domains()[2], "seedance_reference_cleanup_delay", lambda *_: 0)(
                kind, payload, e)
...
def reaper():
    while True:
        ...
        try:
            ...
            pass
        try:
            _domains()[2].retry_pending_seedance_cleanups()
        except Exception:
            pass
        time.sleep(60)
...
def reclaim_orphaned_running():
    reclaimed = startup_recovery.reclaim_orphaned_running(
        jdb=jdb,
...
class H(BaseHTTPRequestHandler):
    ...
    def do_POST(self):
        ...
        if kind is not None:
            ...
            try:
                ...
                if is_still_route:
                    ...
                    if (isinstance(body, dict)
                            and str(body.get("channel") or "grok").strip().lower() == "micro"
                            and not video_domain.seedance_video_health_enabled(feature_flags)):
                        return self._send(503, {
                            "detail": "Seedance 通道暂未开启（未扣点）",
                            "code": "seedance_unavailable",
                            "retry_after_ms": 60000,
                        })
                    body = video_domain.validate_xiaole_video_payload(
                        body, user["username"])
            ...
            except video_domain.SeedanceReferenceUnavailable as e:
            ...
            with _submission_lock:
                ...
                staged_ref_keys = []
                if kind == "xiaole_video":
                    precheck_error = video_domain.xiaole_reference_precheck(
                        kind, body, cost, user.get("points"))
                    if precheck_error:
                        _idempotency_abort(user["username"], p, idem_key)
                        return self._send(*precheck_error)
                    # The idempotency row is already claimed.  Release the
                    # global submission lock around the COS network upload so
                    # one storyboard cannot block unrelated paid submissions.
                    _submission_lock.release()
                    try:
                        staged_ref_keys, staging_error = (
                            video_domain.stage_xiaole_video_references(
                                kind, body, user["username"], idem_key))
                    finally:
                        _submission_lock.acquire()
                    if staging_error:
                        _idempotency_abort(user["username"], p, idem_key)
                        return self._send(*staging_error)
                    if is_shutting_down():
                        video_domain.cleanup_staged_seedance_references(
                            staged_ref_keys)
                        _idempotency_abort(user["username"], p, idem_key)
                        return self._send(503, {
                    ...
                    # Capacity may have changed while the network upload was
                    # in flight.  Recheck every paid admission gate.
                    limit_hit = _user_video_submit_limit(
                        kind, body, user["username"], cost)
                    active_jobs = _user_active_job_count(user["username"])
                    if limit_hit or active_jobs >= MAX_USER_ACTIVE_JOBS:
                        video_domain.cleanup_staged_seedance_references(
                            staged_ref_keys)
                        _idempotency_abort(user["username"], p, idem_key)
                        if limit_hit:
                    ...
                    if is_still_route:
                        ...
                        staged_association = (
                            (lambda connection, job_id:
                                video_domain.link_staged_seedance_references(
                                    connection, staged_ref_keys, job_id))
                        ...
                        jid, points_left = jobs_store.create_paid_job(
                            ...
                            before_commit=staged_association or still_association,
    ...
    def do_GET(self):
        ...
        if p == "/api/gen/health":
            return self._send(200, {"ok": True, "service": "huangque-content", "caps": list(HANDLERS), "job_workers": JOB_WORKERS, "fast_job_workers": FAST_JOB_WORKERS, "talking_job_workers": TALKING_JOB_WORKERS, "image_job_workers": IMAGE_JOB_WORKERS, "job_queue_max": JOB_QUEUE_MAX, "talking_job_queue_max": TALKING_JOB_QUEUE_MAX,
                                    ...
                                    "sora_video_enabled": bool(getattr(video_domain, "sora_video_is_open", lambda: False)() and OPENAI_KEY and feature_flags.is_enabled("sora_video")),
                                    "omni_video_enabled": bool(getattr(video_domain, "omni_video_is_open", lambda: False)() and feature_flags.is_enabled("omni_video")), "seedance_video_enabled": bool(video_domain.seedance_video_is_open() and feature_flags.is_enabled("seedance_video")), "seedance_upscale_enabled": bool(getattr(video_domain, "seedance_upscale_is_open", lambda: False)() and feature_flags.is_enabled("seedance_video")), "reverse_remake_video_channel": video_domain.reverse_remake_video_channel(feature_flags), "seedance_reference_images_enabled": video_domain.seedance_reference_upload_is_open(),
